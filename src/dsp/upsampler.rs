use crate::{
    audio_file::types::Signal, constants::TARGET_SAMPLE_RATE, dsp::Upsampler,
...
impl Upsampler {
            ...
            let ratio = match u8::try_from(TARGET_SAMPLE_RATE / source_sample_rate) {
                ...
                Err(e) => {
                    return Err(SonicProbeError {
                        ...
                        message: format!(
                            "Upsample ratio {}x is too large for u8: {e:?}",
    ...
    #[inline]
    pub fn submit(&self, window: &Signal, start: usize) -> Result<Vec<f64>, SonicProbeError> {
...
fn catmull_rom_interpolation(
    ...
    let Some(&y_minus1) = window.get(start) else {
        return Err(SonicProbeError {
            location: format!("{}:{}", file!(), line!()),
            message: format!("index {start} out of boundaries"),
        });
    };
...
mod tests {
    ...
    fn upsampler_boundary_and_indexing() {
        let upsampler = Upsampler::new(48000).unwrap(); // multipier = 4
        ...
        let results: Vec<f64> = upsampler.submit(&signal, 1).unwrap();
        ...
        let panic_result = std::panic::catch_unwind(|| {
            let _: Vec<f64> = upsampler.submit(&short_signal, 1).unwrap();
        });
    ...
    fn upsampler_interpolation_correctness() {
        let upsampler = Upsampler::new(96000).unwrap(); // multipier = 2

        // Linear signal for predictable interpolation
        let signal: Signal = Arc::from([0.0, 10.0, 20.0, 30.0]);

        let results: Vec<f64> = upsampler.submit(&signal, 0).unwrap();
